	...
	public int insert(Member m) {
		...
		try {// 捕捉异常
			String sql = "Insert into tb_Member (UserName,TrueName,PassWord,City,address,postcode,"
					+ "CardNO,CardType,Tel,Email,accountType) values('"
					...
					+ "','"+chStr.chStr(m.getAccountType())+"')";// 用于实现保存会员信息的SQL语句
	...
	public int update(Member m) {
		...
		try {
			String sql = "update tb_member set TrueName='" + chStr.chStr(m.getTruename()) + "',UserName='"
					...
					+ chStr.chStr(m.getEmail()) + "',accountType='" + chStr.chStr(m.getAccountType()) + "' where ID=" + m.getID();
