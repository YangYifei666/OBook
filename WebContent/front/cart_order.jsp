...
<%
	if (session.getAttribute("cart") == "") {//判断购物车对象是否为空
		out.println(
				"<script language='javascript'>alert('You don't buy anyone!');window.location.href='index.jsp';</script>");
	}
	...
	if (Username != "") {
		try {//捕捉异常
			...
			if (!rs_user.next()) {//如果获取的账户名称在会员信息表中不存在（表示非法会员）
				...
				out.println(
						"<script language='javascript'>alert('Please login first!');window.location.href='index.jsp';</script>");
				return;//返回
			} else {//如果合法会员，则保存订单
				...
				float price = (float) 0.0;//定义保存商品价格的变量
				...
				for (int i = 0; i < cart.size(); i++) {
					...
					price = mygoodselement.price;//获取商品价格
					number = mygoodselement.number;//获取商品数量
					sum = price * number;//计算商品金额
					str = "insert into tb_order_Detail (orderID,goodsID,price,number) values(" + orderID + ","
							+ ID + "," + price + "," + number + ")";//插入订单明细的SQL语句
					temp = conn.executeUpdate(str);//保存订单明细
					Totalsum = Totalsum + sum;//累加合计金额
					ResultSet rs=conn.executeQuery( "select num from tb_goods where ID="+ID);
					rs.last();
					int count=rs.getInt(1);
					count=count-number;
					str1="update tb_goods set num="+count+"where ID="+ID;
					temp= conn.executeUpdate(str1);
				...
				if (!flag) {//如果订单无效
					out.println("<script language='javascript'>alert('Fail Order！');history.back();</script>");
				} else {
					session.removeAttribute("cart");//清空购物车
					out.println("<script language='javascript'>alert('Successful Order Your order number is [" + orderID
